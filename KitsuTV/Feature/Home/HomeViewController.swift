...
class HomeViewController: UIViewController {
  ...
  private func setupCollectionView() {
    ...
    self.registerCollectionViewLayout()
...
//    )
  ...
  private func registerCollectionViewLayout() {
  ...
  fileprivate func makeRecommendSection(_ inset: NSDirectionalEdgeInsets) -> NSCollectionLayoutSection? {
    let layoutSize = NSCollectionLayoutSize(
      ...
      heightDimension: .absolute(
        HomeRecommendContainerCell.height(homeRecommendViewModel: self.homeViewModel.recommendViewModel)
      )
    )
    let item = NSCollectionLayoutItem(layoutSize: layoutSize)
    let group = NSCollectionLayoutGroup.vertical(
  ...
  fileprivate func makeFooterSection() -> NSCollectionLayoutSection? {
    ...
    let group = NSCollectionLayoutGroup.vertical(
      layoutSize: layoutSize,
      subitems: [.init(layoutSize: .init(widthDimension: .absolute(0.1), heightDimension: .absolute(0.1)))
    ])
    let section = NSCollectionLayoutSection(group: group)
